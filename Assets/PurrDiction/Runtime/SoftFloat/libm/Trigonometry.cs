...
namespace PurrNet.Prediction
{
    public static partial class MathS
    {
        ...
        public static sfloat Sin(sfloat x)
        {
            ...
            var piMinusX = sfloat.FromRaw(pi) - x;
            var result = (sfloat)16.0f * x * piMinusX / (sfloat.FromRaw(pi_squared_times_five) - (sfloat)4.0f * x * piMinusX);
        ...
        public static sfloat Hypot(sfloat x, sfloat y)
        {
            ...
            var a = sfloat.FromRaw((uint)ha); /* a <- |a| */
            var b = sfloat.FromRaw((uint)hb); /* b <- |b| */
            ...
            if (hb < 0x26800000)
            {
                ...
                if (hb <= 0x007fffff)
                {
                    ...
                    var t1 = sfloat.FromRaw(0x3f000000); /* t1=2^126 */
                    b *= t1;
                    a *= t1;
                    unchecked
                    {
            ...
            if (w > b)
            {
                var t1 = sfloat.FromRaw((uint)ha & 0xfffff000);
                var t2 = a - t1;
            ...
            {
                a += a;
                var y1 = sfloat.FromRaw((uint)hb & 0xfffff000);
                var y2 = b - y1;
                var t1 = sfloat.FromRaw((uint)ha + 0x00800000);
                var t2 = a - t1;
            ...
            if (k != 0)
            {
                var t1 = sfloat.FromRaw(0x3f800000 + (k << 23));
                return t1 * w;
            }
        ...
        private static readonly uint[] ATAN_HI = {
        ...
        private static readonly uint[] ATAN_LO = {
        ...
        private static readonly uint[] A_T = {
        ...
        public static sfloat Atan(sfloat x)
        {
            ...
            if (ix >= 0x4c800000)
            {
                ...
                var x1p_120 = sfloat.FromRaw(0x03800000); // 0x1p-120 === 2 ^ (-120)
            ...
            var s1 = z * (sfloat.FromRaw(A_T[0]) + w * (sfloat.FromRaw(A_T[2]) + w * sfloat.FromRaw(A_T[4])));
            var s2 = w * (sfloat.FromRaw(A_T[1]) + w * sfloat.FromRaw(A_T[3]));
            if (id < 0)
            {
                return x - x * (s1 + s2);
            }
        ...
        public static sfloat Atan2(sfloat y, sfloat x)
        {
            ...
            if (iy == 0)
            {
                return m switch
                {
                    ...
                    2 => sfloat.FromRaw(pi) /* atan(+0,-anything) = pi */,
                    _ => -sfloat.FromRaw(pi)
                };
            }
            ...
            switch (ix)
            {
                ...
                /* when x is INF */
                case 0x7f800000 when iy == 0x7f800000:
                    return m switch
                    {
                        0 => sfloat.FromRaw(pi_over_4) /* atan(+INF,+INF) */,
                        1 => -sfloat.FromRaw(pi_over_4) /* atan(-INF,+INF) */,
                        2 => sfloat.FromRaw(pi_times_3_over_4) /* atan(+INF,-INF)*/,
                        _ => -sfloat.FromRaw(pi_times_3_over_4)
                    };

                case 0x7f800000:
                    return m switch
                    {
                        0 => sfloat.zero /* atan(+...,+INF) */,
                        1 => -sfloat.zero /* atan(-...,+INF) */,
                        2 => sfloat.FromRaw(pi) /* atan(+...,-INF) */,
                        _ => -sfloat.FromRaw(pi)
                    };
            }
            ...
            return m switch
            {
                0 => z /* atan(+,+) */,
                1 => -z /* atan(-,+) */,
                2 => sfloat.FromRaw(pi) - (z - sfloat.FromRaw(PI_LO_U32)) /* atan(+,-) */,
                _ => z - sfloat.FromRaw(PI_LO_U32) - sfloat.FromRaw(pi)
            };
        }
        ...
        public static sfloat Acos(sfloat x)
        {
            ...
            {
                var p = z * (sfloat.FromRaw(P_S0_U32) + z * (sfloat.FromRaw(P_S1_U32) + z * sfloat.FromRaw(P_S2_U32)));
                var q = (sfloat)1.0f + z * sfloat.FromRaw(Q_S1_U32);
                return p / q;
            }
            ...
            {
                /* |x| >= 1 or nan */
                case >= 0x3f800000 and 0x3f800000:
                {
                    if (hx >> 31 != 0)
                    {
                ...
                case >= 0x3f800000:
                    return sfloat.NaN;
                /* |x| < 0.5 */
                case < 0x3f000000 and <= 0x32800000:
                    /* |x| < 2**-26 */
            ...
            s = Sqrt(z);
            hx = s.rawValue;
            var df = sfloat.FromRaw(hx & 0xfffff000);
            var c = (z - df * df) / (s + df);
            w = r(z) * s + c;
